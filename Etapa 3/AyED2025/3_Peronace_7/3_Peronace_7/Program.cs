    ...
    internal class Program
    {
        ...
        static void MostrarTodos(string[] nombre, string[] saga, int[] fuerza, int[] defensa, bool[] esheroe, int contador)
        {
            ...
            for (int i = 0; i < contador - 1; i++)
            {
                ...
                {
                    if (string.Compare(nombre[indices[i]], nombre[indices[j]]) > 0)
                    {
                        int temp = indices[i];
                        indices[i] = indices[j];
                        indices[j] = temp;
            ...
            for (int i = 0; i < contador; i++)
            {
                ...
                Console.WriteLine("Nombre:" + nombre[indi]);
                Console.WriteLine("Saga:" + saga[indi]);
                Console.WriteLine("Fuerza:" + fuerza[indi]);
                Console.WriteLine("Defensa:" + defensa[indi]);
                if (esheroe[i])
                {
                    Console.WriteLine("Estado:Heroe");
                }
